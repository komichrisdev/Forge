...
export function boundedText(value, limit = MAX_TEXT) {
...
function clampPage({ limit = 20, offset = 0 }) {
    if (!Number.isInteger(limit) || !Number.isInteger(offset))
        throw new PublicError("Pagination values must be integers.");
    return { limit: Math.max(1, Math.min(MAX_PAGE, limit)), offset: Math.max(0, Math.min(100_000, offset)) };
}
function timeOf(events, name) {
...
export class SwarmData {
    ...
    runDir(runId) {
        if (!RUN_ID.test(runId) || runId === "." || runId === "..")
            throw new PublicError("Invalid run ID.");
        const path = resolve(this.runRoot, runId);
        if (!path.startsWith(this.runRoot + sep))
            throw new PublicError("Invalid run ID.");
        try {
            ...
            throw new PublicError("Run not found.");
        }
        return path;
    }
    read(path, shouldRedact = true) {
        const target = resolve(path);
        if (!target.startsWith(this.runRoot + sep))
            throw new PublicError("Artifact path is outside the run directory.");
        try {
    ...
    readJson(path) {
        ...
        return parsed && typeof parsed === "object" && !Array.isArray(parsed) ? redact(parsed) : {};
    }
    events(dir) {
    ...
    runIds() {
        try {
            ...
            throw new PublicError("Swarm run directory is unavailable.");
        }
    }
    runBrief(runId) {
    ...
    listRuns(filters = {}) {
        ...
        if (filters.dateFrom)
            runs = runs.filter((run) => String(run.createdAt) >= filters.dateFrom);
        if (filters.dateTo)
            runs = runs.filter((run) => String(run.createdAt) <= filters.dateTo);
    ...
    status() {
        const db = this.db();
        try {
            const modelCount = Number(db.prepare("SELECT count(*) n FROM models").get().n);
            const attempts = db.prepare("SELECT status, elapsed_ms FROM task_attempts ORDER BY attempted_at DESC, id DESC LIMIT 100").all();
            const qualityEvents = Number(db.prepare("SELECT count(*) n FROM quality_events").get().n);
            const benchmarks = Number(db.prepare("SELECT count(*) n FROM benchmark_results").get().n);
    ...
    runSummary(runId) {
        ...
        return {
            ...
            workers: (Array.isArray(task.workers) ? task.workers : []).map((item) => {
                const worker = item;
                return { role: worker.name || "", model: worker.model || "", selectionReason: boundedText(worker.selection_reason, 400).text };
            }),
            judge: { model: task.judge?.model || "" }, failures,
    ...
    runDetails(runId) {
        ...
        const final = this.readJson(`${dir}/final.json`);
        const events = this.events(dir);
        const readGroup = (subdir, allowed) => {
        ...
        const workerCards = (Array.isArray(task.workers) ? task.workers : []).map((item) => {
            ...
            return {
                ...
                reliabilityPenalty: profile.reliability?.reliabilityPenalty ?? null,
                reliability: profile.reliability || {},
                roleQualityScore: profile.quality?.roleScores?.[role] ?? null,
        ...
        if (size > MAX_META) {
            delete detail.context.sent;
            delete detail.workers;
            for (const item of Object.values(detail.prompts)) {
                if (item && typeof item === "object")
                    item.text = boundedText(item.text, 8_000).text;
            }
            for (const card of detail.workerCards) {
                const response = card.response;
                response.text = boundedText(response.text, 16_000).text;
                response.truncated = true;
            }
            const judge = detail.judge;
            judge.response.text = boundedText(judge.response.text, 16_000).text;
            detail.finalMarkdown.text = boundedText(detail.finalMarkdown.text, 16_000).text;
            detail.payloadTruncated = true;
            detail.originalPayloadBytes = size;
            if (Buffer.byteLength(JSON.stringify(detail)) > MAX_META) {
                detail.prompts = {};
                for (const card of detail.workerCards)
                    card.response.text = "[TRUNCATED: payload limit]";
            }
        }
        return { concise, detail };
    }
    evidenceForRun(runId) {
        const db = this.db();
        try {
            return {
                attempts: db.prepare("SELECT model_id, role, mode, attempted_at, status, elapsed_ms, retry_count FROM task_attempts WHERE run_id=? ORDER BY id").all(runId),
                quality: db.prepare("SELECT model_id, role, mode, category, severity, judge_caught, reached_final, codex_verified, note, created_at FROM quality_events WHERE run_id=? ORDER BY id").all(runId),
                benchmarks: db.prepare("SELECT benchmark_id, benchmark_version, model_id, role, mode, checks, dimensions, evaluator_source, note, evaluated_at FROM benchmark_results WHERE run_id=? ORDER BY id").all(runId).map((row) => ({ ...row, checks: json(String(row.checks), {}), dimensions: json(String(row.dimensions), {}) })),
            };
        }
        finally {
            db.close();
    ...
    modelMetrics(db, modelId) {
        const attempts = db.prepare("SELECT status, elapsed_ms, attempted_at FROM task_attempts WHERE model_id=? ORDER BY attempted_at DESC, id DESC LIMIT 20").all(modelId);
        ...
        const events = db.prepare("SELECT role, category, severity, run_id, note, created_at FROM quality_events WHERE model_id=? ORDER BY created_at DESC LIMIT 100").all(modelId);
        const benchmarks = db.prepare("SELECT benchmark_id, run_id, role, dimensions, checks, note, evaluated_at FROM benchmark_results WHERE model_id=? ORDER BY evaluated_at DESC LIMIT 20").all(modelId);
        const evidence = new Set([...events, ...benchmarks].map((row) => String(row.run_id || `${row.benchmark_id}:${row.evaluated_at}`))).size;
        const scores = {};
        for (const row of benchmarks)
            for (const value of Object.values(json(String(row.dimensions), {})))
                if (typeof value === "number")
                    (scores[String(row.role)] ||= []).push(value);
        ...
        const categories = events.reduce((out, row) => { const key = String(row.category); out[key] = (out[key] || 0) + 1; return out; }, {});
    ...
    listModels(filters = {}) {
        ...
        try {
            ...
            const params = [];
            if (filters.search) {
                where.push("(model_id LIKE ? ESCAPE '\\' OR notes LIKE ? ESCAPE '\\')");
            ...
            if (filters.chatCompatible !== undefined) {
                where.push(filters.chatCompatible ? "kind='chat'" : "kind<>'chat'");
            }
            if (filters.family) {
                where.push("family=?");
            ...
            const rows = db.prepare(`SELECT * FROM models ${where.length ? `WHERE ${where.join(" AND ")}` : ""} ORDER BY enabled DESC, model_id LIMIT ? OFFSET ?`).all(...params, roleFilter ? 100_000 : page.limit, roleFilter ? 0 : page.offset);
            ...
            if (filters.recommendedRole)
                models = models.filter((model) => model.recommendedRoles.includes(filters.recommendedRole));
            const count = filters.recommendedRole ? models.length : Number(db.prepare(`SELECT count(*) n FROM models ${where.length ? `WHERE ${where.join(" AND ")}` : ""}`).get(...params).n);
            if (roleFilter)
                models = models.slice(page.offset, page.offset + page.limit);
    ...
    model(modelId) {
        if (!modelId || modelId.length > 300)
            throw new PublicError("Invalid model ID.");
        const db = this.db();
        try {
            const row = db.prepare("SELECT * FROM models WHERE model_id=?").get(modelId);
            if (!row)
